...
export class CategoryComponent implements OnInit {
  ...
  constructor(private categoryService: CategoryService, private productService: ProductService, private cartService: CartService, private favoriteService: FavoriteService, private userAuthService: UserAuthService, private activatedRoute: ActivatedRoute) { }

  ngOnInit(): void {
    this.activatedRoute.queryParams.subscribe(params => {
      this.id = params['id'];
      this.name = params['name'];
      this.filters.categoryDetailId = this.id
    });
  ...
  private findAllProduct() {
    ...
    this.categoryService.findAllProductByCategory(this.id, this.pageNumber, sizePage).subscribe((response: any) => {
  ...
  private handleFilter() {
    ...
    this.categoryService.maxPrice(this.id).subscribe((response: any) => {
    ...
    btnCloseFilter.addEventListener("click", () => {
      ...
      this.categoryService.maxPrice(this.id).subscribe((responseD: any) => {
  ...
  private filter() {
    ...
    this.categoryService.filterProductByCategory(this.filters).subscribe((response: any) => {
      for (let i = 0; i < response.products.length; i++) {
        this.handleProductDetail(response.products[i])
