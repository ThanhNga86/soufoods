    ...
    if (filter) {
      ...
      filterTable.innerHTML += `
        <div class="form-check form-switch">
          ...
          <label class="form-check-label label-filter">Trạng thái danh mục</label>
        </div>
        <div class="form-check form-switch">
          <input class="form-check-input input-filter" type="checkbox" key="activeCd" value-checked="true" value-unchecked="false" role="switch" style="cursor: pointer; box-shadow: none;">
          <label class="form-check-label label-filter">Trạng thái danh mục chi tiết</label>
        </div>
      `
    ...
    this.adminCategoriesService.findAllCategoryDetailByCategory(id).subscribe((response: any) => {
      ...
      for (let i = 0; i < response.length; i++) {
        ...
        typeCategory.innerHTML += (response[i].active == true) ?
          '<div class="form-group"><label>Trạng thái</label><select class="form-select inpForm inpActiveCd"><option value="true">Đang bán</option><option value="false">Ngừng bán</option></select></div>' :
          '<div class="form-group"><label>Trạng thái</label><select class="form-select inpForm inpActiveCd"><option value="true">Đang bán</option><option value="false" selected>Ngừng bán</option></select></div>'
    ...
    this.adminCategoriesService.findById(id).subscribe((response: any) => {
      ...
      this.adminCategoriesService.findAllCategoryDetailByCategory(id).subscribe((response: any) => {
        ...
        for (let i = 0; i < response.length; i++) {
          ...
          `
          typeCategory.innerHTML += (response[i].active == true) ?
            '<div class="form-group mx-3"><label>Trạng thái</label><select class="form-select inpForm inpActiveCd"><option value="true">Đang bán</option><option value="false">Ngừng bán</option></select></div>' :
            '<div class="form-group mx-3"><label>Trạng thái</label><select class="form-select inpForm inpActiveCd"><option value="true">Đang bán</option><option value="false" selected>Ngừng bán</option></select></div>'
          ctnTypes.appendChild(typeCategory)
