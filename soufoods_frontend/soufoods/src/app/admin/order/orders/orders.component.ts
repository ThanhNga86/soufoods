...
export class OrdersComponent implements OnInit{
  ...
  private findAll() {
    ...
    this.adminOrderService.findAll(this.pageNumber, sizePage).subscribe((response: any) => {
  ...
  private handleFilter() {
    ...
    if (filter) {
      ...
      const filterStatus: any = document.createElement('select');
      filterStatus.classList.add('form-select', 'formFilterCategory', 'mt-2');
      filterStatus.style.boxShadow = 'none'
      filterStatus.innerHTML += `
        <select class="form-select filterStatus">
          <option value="null">Chọn trạng thái đơn hàng</option>
      ...
      filterStatus.addEventListener("change", () => {
        var optionFirst = filterStatus.querySelectorAll("option")[0]
        optionFirst.innerHTML = filterStatus.value == 'null' ? 'Chọn trạng thái đơn hàng' : 'Tắt lọc'
      ...
      filterTable.appendChild(filterStatus);
  ...
  private filter() {
    ...
    this.adminOrderService.filter(this.filters).subscribe((response: any) => {
      this.orders = response.orders
      this.total = response.total
      this.totalPage = Array.from({ length: response.totalPage }, (_, index) => index + 1);
