...
export class AddVoucherComponent implements OnInit {
  ...
  private add() {
    ...
    const btnSave: any = document.querySelector(".btnSave")
    const emailType: any = document.querySelector(".emailType")
    ...
    const addVoucher = (userId: any, i: number, length: number) => {
      this.adminVouchersService.createDiscount().subscribe((response: any) => {
        if (response.status == '200') {
          if (userId != null) {
            formData.set("user", userId)
          }
          formData.set("discountCode", response.discountCode)
          ...
          // Nếu gửi cho tất cả thì ko đc gửi email vì do quả tải
          if (emailType.value == 'false') {
    ...
    if (emailType.value == "true") {
      this.adminUsersService.findAllUser().subscribe((response: any) => {
        for (let i = 0; i < response.length; i++) {
          addVoucher(response[i].id, i, response.length - 1)
        }
      })
    } else {
      const emailAdded = document.querySelectorAll(".emailAdded")
      if (emailAdded.length && emailAdded.length != 0) {
        for (let i = 0; i < emailAdded.length; i++) {
          const id = emailAdded[i].getAttribute("id")
          addVoucher(id, i, emailAdded.length - 1)
        }
      } else {
        addVoucher(null, 0, 0)
      }
